...
bool MP1Node::recvCallBack(void *env, char *data, int size ) {
    ...
    memcpy(incomingMsgAddress.addr, data, sizeof(incomingMsgAddress.addr));  //  address(member) of where message came from
    ...
    switch(receivedMsg->msgType)
    {
        case(JOINREQ):
            ...
            emulNet->ENsend(&this->memberNode->addr, &incomingMsgAddress, (char *)outgoingMsg, outgoingMsgSize);  // send JOINREP back to node letting know added
            free(outgoingMsg);
            break;

        case(JOINREP):
            cout << "join reply rec'd" << endl;
            memberNode->inGroup = true;
