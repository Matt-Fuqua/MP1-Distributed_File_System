...
//            memberNode->heartbeat +=1;
        ...
        case(GOSSIP):       // member rec'd an updated ML to compare to their own
          ...
          //      cout << "checking to see if temp ID: " << tempID << " is in members " << (int)memberNode->addr.addr[0] << endl;
                ...
                for(thisMemberPosition = memberNode->memberList.begin();
                    ...
                    thisMemberPosition++)
                {
 //                   cout << "List ID  " << thisMemberPosition->id << " passed ID " << tempID << endl;
 ...
 //                       cout << "checking same ID, do not add. This members ID " <<thisMemberPosition->id << " passed ID " << tempID << endl;
                ...
                {
             //       cout << "adding member: " << tempID << " to member ship of " << nodeID << " from " << msgFromID << 
             //        " with heartbeat " << tempHB << endl; 
...
// send msg fail to all other members
 ...
 vector<MemberListEntry>::iterator memberPosition;
    ...
    // go through each item in member list. if heartbeat has not been upated  for x number of time, declare member as failed
    // may need to add confirmation step
        ...
        memberPosition++)
    {
        ...
        // ****** where i left off 9/24 ************************************
        // !!!!!!!!!!!!!!!!!!!!!!!!! NOT REMOVING !!!!!!!!!!!!!!!!!!!!!!!!
        if(tempTime + TFAIL+100 < par->getcurrtime())       // possible failure
        {
...
// next step is to implement the fail detector
// random may be issue with member selector
